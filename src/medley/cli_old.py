...
def ensemble(case_file: str, models: tuple, output_dir: Optional[str], format: str, no_cache: bool, 
    ...
    with console.status("[bold green]Initializing Medley Ensemble...") as status:
        ...
        try:
            results = orchestrator.run_ensemble_analysis(
                ...
                use_paid_models=use_paid_models,
                max_paid_models=max_paid,
    ...
    console.print("\n[bold magenta]Generating consensus report...[/bold magenta]")
    ...
    if report_type == "comprehensive":
        report_prefix = "comprehensive_report"
    elif report_type == "enhanced":
        report_prefix = "enhanced_report"
    else:
        report_prefix = "consensus_report"
    report_filename = f"{report_prefix}_{case_id}_{timestamp}.{extension}"
    ...
    # If comprehensive report type, save the enhanced JSON data
    if report_type == "comprehensive" and isinstance(report, dict):
        # Save enhanced JSON with comprehensive fields
        json_filename = f"{report_prefix}_{case_id}_{timestamp}.json"
        json_path = Path(output_dir) / json_filename
        with open(json_path, 'w') as f:
            json.dump(report, f, indent=2, default=str)
        console.print(f"[green]✓ Enhanced JSON saved to: {json_path}[/green]")
